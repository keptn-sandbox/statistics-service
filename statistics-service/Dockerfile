...
WORKDIR /src/statistics-service
...
RUN GOOS=linux go build -ldflags '-linkmode=external' $BUILDFLAGS -v -o statistics-service
...
COPY --from=builder /src/statistics-service/statistics-service /statistics-service
